<template>
  <el-container>
    <el-header></el-header>
    <el-main>
      <el-dialog title="Downloading..." :visible.sync="showDialog" :show-close="false" :close-on-click-modal="false" width="300px">
        ...
        <div v-else>
          <el-progress :text-inside="true" :stroke-width="20" color="rgba(201, 23, 30, 1.0)" :percentage="progress"></el-progress>
      ...
      <el-form>
        ...
        <div style="text-align: right;">
          <el-row>
            <el-switch v-model="doArchive" active-text="ZIPにまとめて保存"></el-switch>
          </el-row>
          <el-row>
...
<script>
  ...
  ipcRenderer.on('start-downloading', () => {
  ...
  export default {
    ...
    methods: {
      download() {
        ipcRenderer.send('start-download', data.url, data.doArchive);
      },
      cancel() {
...
<style>
  ...
  .sk-rect.sk-rect4,
  .sk-rect.sk-rect5 {
    background-color: rgba(229, 228, 230, 1.0);
  }
